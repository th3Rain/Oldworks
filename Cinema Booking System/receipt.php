...
<body bgcolor="white">
<h2 Align="left" font face="Courier New, Courier, monospace" Color="Green">&nbsp;&nbsp;NewCinema Movie Ticket Reservation</br></br>&nbsp;&nbsp;&nbsp;COSTUMER'S COPY</font></h2>
...
<?php
$query=mysqli_query($connection, "SELECT * FROM `movies` ORDER BY `movies`.`ID`  DESC") or die(mysqli_error($connection));
{
...
<h1 align="left"><font face="arial" Color="black" size="2px">&nbsp;&nbsp;This is your copy. Keep this part in a safe place. The following are requirements when claiming your ticket:&nbsp;&nbsp;</br></font><font size="1">&nbsp;&nbsp;*&nbsp;This Ticket Voucher (Costumer's copy)<br>&nbsp;&nbsp;*&nbsp;Personally claim your movie ticket after reservation on working days in any NewCinema ticket booth nationwide. Working days are from Monday to Sunday.<br>&nbsp;&nbsp;This document is valid after reservation until <?php echo date("M-d-Y"); ?>&nbsp;between&nbsp;<?php

$query=mysqli_query($connection, "SELECT * FROM `movies` ORDER BY `movies`.`Ticket_Number`  DESC") or die(mysqli_error($connection));
{
	$row=mysqli_fetch_array($query);
	echo "".$row['Time']."";
}
